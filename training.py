...
class CustomCallback(keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        progress = (
            (nTrain_batches * (self.curEpoch - 1) + batch + 1)
            * 1.0
            / (nTrain_batches * self.opt.nEpochs)
        )
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        sys.stdout.write(
            "Epoch: {}/{} | Time: {} | Train: LR {}  Loss {:.3f}%  Acc {:.3f}% | Val: Loss {:.3f}%  Acc(top1) {:.3f}%\n".format(
                ...
                logs.get("accuracy") if logs.get("accuracy") else 0,
                val_loss,
                val_acc,
            )
        )
    ...
    def validate(self, model):
        y_pred = None
        y_target = None
        for batchIndex in range(
            math.ceil(len(self.val_gen.data) / self.val_gen.batch_size)
        ):
    ...
    def compute_accuracy(self, y_pred, y_target):
        ...
        y_pred = y_pred.reshape(
            y_pred.shape[0] // self.opt.nCrops, self.opt.nCrops, y_pred.shape[1]
        )
